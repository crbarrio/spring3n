...
function createButtonListeneres(mode) {
...
function setupPagination(totalItems, mode) {
    ...
    for (let index = 1; index <= pages; index++) {
        ...
        let paginatioTypeClass = mode === 'custom' ? 'pageGeneralButton' : 'pageButton'
        let activeAttr = '';
        let buttonClass = paginatioTypeClass + ' font-medium leading-5 text-sm px-3 py-2';
    ...
    createButtonListeneres(mode)
}

// Funció per a la visualització dels resultats i la paginació (a implementar)
export function displayResults(items, totalItems, mode) {
    ...
    if (mode === 'custom') {
        ...
        list.forEach(item => {
            ...
            output += `
                <div class="generalItems text-start border rounded-sm p-2 shadow-sm hidden" data-page="${page}">
    ...
    setupPagination(totalItems, mode)
}